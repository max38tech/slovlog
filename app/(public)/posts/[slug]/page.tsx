  ...
  return (
    <article className="pb-24">
      ...
      <header className="max-w-4xl mx-auto px-4 sm:px-6 pt-12 pb-8">
        <div className="flex items-center justify-between mb-8">
          ...
          {post.location && (
            <Link
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slovenia-green/10 text-slovenia-green text-xs font-semibold hover:bg-slovenia-green/20 transition-colors"
            >
        ...
        {/* Subtitle / Excerpt */}
        {post.excerpt && (
          <p className="text-lg sm:text-xl text-slate-600 font-sans mt-4 leading-relaxed">
            {post.excerpt}
          </p>
        )}

        {/* Metadata Strip */}
        <div className="flex flex-wrap items-center gap-4 sm:gap-6 text-xs text-slate-500 pt-6 mt-6 border-t border-slate-200">
          <span className="flex items-center gap-1.5">
            <Calendar className="w-4 h-4 text-slovenia-blue" />
            {post.trip_date}
          </span>
          <span>•</span>
          <span className="flex items-center gap-1.5">
          ...
          <span>•</span>
          <span className="flex items-center gap-1.5">
            <Compass className="w-4 h-4 text-slovenia-green-leaf" />
            Slovenia Journey
          </span>
        </div>
      </header>
      ...
      {post.cover_image && (
        <div className="max-w-5xl mx-auto px-4 sm:px-6 mb-12">
          <div className="relative aspect-[16/9] w-full rounded-3xl overflow-hidden shadow-lg bg-slate-100 border border-slate-200">
            <Image
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        <div className="mt-16 pt-10 border-t border-slate-200 grid grid-cols-1 sm:grid-cols-2 gap-4">
          {prevPost ? (
            <Link
              href={`/posts/${prevPost.slug}`}
              className="p-5 rounded-2xl border border-slate-200 hover:border-slovenia-blue/40 bg-white hover:bg-slate-50/80 transition-all text-left group"
            >
          ...
          {nextPost && (
            <Link
              ...
              className="p-5 rounded-2xl border border-slate-200 hover:border-slovenia-blue/40 bg-white hover:bg-slate-50/80 transition-all text-right group sm:col-start-2"
            >
              <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider flex items-center justify-end gap-1 mb-1">
                Next Story
