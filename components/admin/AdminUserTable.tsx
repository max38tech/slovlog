...
export function AdminUserTable({ users }: { users: AdminUserItem[] }) {
  ...
  return (
    <div className="space-y-6">
      {/* Add Admin Form */}
      <div className="bg-white dark:bg-[#131d2e] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm">
        <h2 className="font-universa text-lg font-normal text-slate-900 dark:text-white tracking-[0.06em] uppercase flex items-center gap-2 mb-1">
          <UserPlus className="w-5 h-5 text-slovenia-blue dark:text-blue-400" />
          Authorize New Administrator
        </h2>
        <p className="text-xs text-slate-500 dark:text-slate-400 mb-4">
          Add a collaborator&apos;s Google email address. Once added, they can sign in via Google OAuth at <span className="font-mono text-slate-700 dark:text-slate-300">/admin</span>.
        </p>
        ...
        <form onSubmit={handleAddAdmin} className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              required
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-sm text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-slovenia-blue/20 focus:border-slovenia-blue"
            />
          </div>
                  ...
                  <tr key={user.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-slate-600 dark:text-slate-300 font-bold text-xs uppercase">
                          {user.email.charAt(0)}
                        </div>
                        <div>
                          <p className="font-medium text-slate-900 dark:text-white">{user.email}</p>
                          <p className="text-xs text-slate-400 dark:text-slate-500">Google OAuth Identity</p>
                        </div>
                      </div>
                    </td>
                      ...
                      <span
                        className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-full ${
                          isOwner
                            ...
                            : "bg-blue-50 dark:bg-blue-950/40 text-slovenia-blue dark:text-blue-300 border border-blue-200 dark:border-blue-800/40"
                        }`}
                      >
                        {isOwner ? (
                          <ShieldCheck className="w-3 h-3 text-amber-600 dark:text-amber-400" />
                        ) : (
                          <Shield className="w-3 h-3 text-slovenia-blue dark:text-blue-400" />
                        )}
                    ...
                    <td className="py-4 px-6 text-right">
                      {isOwner ? (
                        ...
                        <button
                          ...
                          disabled={isPending}
                          className="inline-flex items-center gap-1 text-xs text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-950/30 px-2.5 py-1.5 rounded-lg border border-red-200 dark:border-red-900/40 transition-colors"
